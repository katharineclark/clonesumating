...
function swapPhoto(ele) {
	...
	main.src = images[newId].src || 'http://img.consumating.com/photos/'+thisprofileid+'/large/'+newId+'.jpg';
	main.id = 'photo'+newId;


	lower.id = 'photo'+oldId;
	lower.src = images[oldId+',100'].src || 'http://img.consumating.com/photos/'+thisprofileid+'/100/'+oldId+'.jpg';
...
function handleNewTopic(response) {
	try {
		...
		document.getElementById('topicResponses').innerHTML='<div id="responsesArea"></div>';
...
function removeTopic(id) {
	...
	c.push("<div class='infoBoxOptionWrapper'><a href='#' onClick='deleteConversation("+id+");InfoBox.clear(); return false;' class='infoBoxOption'><div class='ibp'>Delete</div></a>");
...
function changeTagline() {
	...
	c.push(_makeButton('smallblue','Update','#','onclick="saveTagline();return false;"'));
	c.push('<br clear="all"/>');
...
function sendInvite(userId,eventId) {
	apiRequest('user.invite','userId='+userId+'&eventId='+eventId,handleInvite);
	return false;
}
function handleInvite(r) {
	var rxml = r.responseXML;
	if (rxml.getElementsByTagName('success').length) {
		alert('Invite sent!');
	} else {
		alert('Sorry, your invite failed to send.');
	}
}
...
function setBehavior(itemId,str) {
	...
	try {
		...
		if (bh[0] == 'wallpaper') {
			...
			displayBehavior(bh[1]);
		} else if (bh[0] == 'theme') {
			displayBehavior(bh[1]);
		} else {
			var ele = document.getElementById('slotimg'+itemId) || document.getElementById('itemimg'+itemId);
			var f = ele.onclick;
			ele.onclick = function() {
				try {
					...
					$('pluginContent').innerHTML = player.show();
...
function displayBehavior(behavior) {
	if (behavior.indexOf('loadScript') > -1) {
		try{
			eval(behavior);
		}catch(e){alert('error dh1: '+e.message)}
	} else {
		document.write('<style>'+behavior+'</style>');
	}
}

function showRockyou(id) {
	var c = new Array();
