...
sub init {
	my $self = shift;
	$self->{headerScript} = qq|<script language="javascript" src="http://update.videoegg.com/js/Player.js"></script>|;
	return $self;
}
...
sub video {
	...
	my $sth = $self->{dbh}->prepare("SELECT path FROM videos WHERE id = ?");
	$sth->execute($id);
warn "ROWS FOR $id? ".$sth->rows;
	return unless $sth->rows;
	my $path = $sth->fetchrow;
warn "FOUDN PATH $path";
	return $path;
}

sub publisher {
	my $self = shift;
	return VE_EmbedPublisher2('saveVideo','BasicConfig','666666','false','test=1',$self->{user}{user}{handle},"http://www.consumating.com/profiles/$self->{user}{user}{linkhandle}",'');
#    return VE_EmbedPublisher2('SwapPlayer','BasicConfig','666666','false','test=1',$self->{user}{user}{handle},"http://www.consumating.com/profiles/$self->{user}{user}{linkhandle}",'');
...
$VE_HOSTNAME_PREFIX = "selfserve1.";
$VE_CLIENT_ID = "gid361/cid1241";
$VE_UPLOAD_PASSWORD = "p2w253rYnvkELmF5wv40";
$VE_DOWNLOAD_PASSWORD = "p2w253rYnvkELmF5wv40";
