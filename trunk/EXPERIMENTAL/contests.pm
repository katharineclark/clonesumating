...
sub displayDefault() {
	...
	my $getContest = $self->{dbh}->prepare("SELECT description,id FROM photo_contest WHERE id=?");
    ...
    while (my $content = $getHotPics->fetchrow_hashref) {
       ...
       my $U = Users->new(dbh => $self->{dbh}, cache => $self->{cache}, userId => $content->{userId}) or next;
       $getContest->execute($content->{'questionId'});
       my $q = $getContest->fetchrow_hashref;

        push(@{$self->{user}{hot_answers}},{content => $content,question=>$q,user=>$U->profile});
...
sub displayPostScreen() {
	...
	$self->{user}{page}{type} = $self->{query}->param('type');
